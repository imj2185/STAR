...
class SpatialEncoderLayer(nn.Module):
    def __init__(self,
        ...
        self.use_full_attention = full_attention
    ...
    def forward(self, x, adj=None):
        ...
        if not self.use_full_attention:
            ...
            msk_adj = FullMask((to_dense_adj(adj) == 1))
            t = self.multi_head_attn(query, key, value, msk_adj)
...
class SpatialFullEncoderLayer(nn.Module):
    ...
    def forward(self, x, att=None):
        ...
        t = self.multi_head_attn(query, key, value, None)
        t = rearrange(t, 'f n h c -> f n (h c)', h=self.heads)
